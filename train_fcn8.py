...
# ROOT_PATH = "/media/ming/DATADRIVE1/Academic/2017_Thesis"
...
def initialize_model(outputChannels, wd=None, modelWeightPaths=None):
    ...
    params = {
        ...
        # "fc_6": {"name": "fc6", "shape": [7,7,512,4096], "std": None, "act": "relu"},
        # "fc_7": {"name": "fc7", "shape": [1,1,4096,4096], "std": None, "act": "relu"},
        #
...
def forward_model(model, outputChannels, imageNamesListPath, imageDir, depthDir, outputSavePath=None):
    with tf.Session() as sess:
        ...
        for imageName in imageNamesList:
            ...
            outputImage = sess.run( [model.prediction], feed_dict={tfBatchImages: [image],
                                                               keepProb: 1.0})
                                            # upscore32argmax
...
def train_model(model, outputChannels, learningRate, trainListPath, trainImageDir, trainDepthDir, trainGTDir, valListPath,
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        with tf.device('/gpu:0'):
            ...
            loss = lossFunction.modelTotalLoss(pred=model.logits, gt=tfBatchGT, outputChannels=outputChannels)
                                                        # upscore32
            # calculate intersection and union of prediction with respect to ground truth
            intersection, union = lossFunction.intersectionAndUnion(pred=model.logits, gt=tfBatchGT,
                                                                    outputChannels=outputChannels)
                                                                                # upscore32
            mid_shape = model.mid_shape
            ...
            while iteration < 1000:
                ...
                for imageName in valImageNamesList:
                    ...
                    lossBatch, intersectionBatch, unionBatch, mshape, ishape= sess.run([loss, intersection, union, mid_shape, input_shape],
                                                             feed_dict={tfBatchImages: [image],
                                                                        tfBatchGT: [gt],
                                                                        keepProb: 1.0})
                    print "shape after deconv: " + str(mshape)
                    print "input shape: " + str(ishape)
                ...
                for imageName in trainImageNamesList:
                    ...
                    # in this case, we execute one trainingoptimization step for each 1000x5000 strip of image. You can modify this feeding
...
if __name__ == "__main__":
    ...
    if train:
        ...
        modelWeightPaths = [ROOT_PATH + "/test/aerial_semantic_models/fcn8_zoning/torontocity_fcn8_zoning3_halfres_170.mat"]
        initialIteration = 171
